...
void case2helper(char *c, item *u, item **dla, int depth, int i, int subsize)
{
	static item *v = NULL;
	static int k;
	static int z=0;
	int l1, l2, index = conv[c[i-1]];
...
//We must be able to find a leaf!
	if(u -> A_son_list[index] == NULL ||						//From Case 2
		...
		case2helper(c, u -> T_parent,dla, depth+1, i, subsize + 1+u->ifsub);
	}
...
void case3helper(char *c, item *u, item **dla, int i)
{
	int l1, l2, depth_diff, index = conv[c[i-1]];
...
void case4helper(char *c, item *u, item **dla, int i)
{
	int l1, l2, depth_diff, index = conv[c[i-1]];
	item *v = u -> A_son_list[index];
	item *temp, *temp2;
	...
	if(depth_diff < 0)		//Assumption: v -> ifsub != 0 && v -> ifsub >= depth_diff
	{
		...
		temp =
		u -> A_son_list[index] =
		v -> T_parent -> T_son_list[ conv[ v -> T_edge ] ] =
		newItem(v -> T_parent, u, v -> T_edge, c[i-1], 0);
		...
		l2 = conv[ v -> substring[ v-> ifsub + depth_diff] ];
...
// Root and Tn+1 An+1 Setting
	...
	dla[end] =	root->T_son_list[conv['$']] =
			root->A_son_list[conv['$']] =
	...
	for(k=end-2; k>=0; k--)
	{
		...
		while(temp != NULL && temp->B[index] != 1)	//Finding Uy
		{
			temp -> B[index] = 1;
			temp = temp->T_parent;
		}
		...
		//Case 2
		else if(temp->A_son_list[index] == NULL ||
			temp -> A_son_list[index] -> T_depth < temp -> T_depth + 1 - temp -> ifsub)			{
			kbyk[i] = 2;
			case2helper(x, temp, dla, 0, i, 0);
		}			

		else if(temp -> A_son_list[index] -> ifleaf != 0)	//case 3
		{
			kbyk[i] = 3;		
